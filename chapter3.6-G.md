在前一小节中我们主要介绍了文档数据库的聚合查询，本小节我们主要介绍图数据库的查询处理相关内容。图数据库的查询处理是指处理图查询语⾔的过程，由图查询引擎负责，其任务是将⽤户提交的图查询语句转换为具体的查询执⾏计划。不同的图数据库拥有不同的图查询语⾔，在本节中我们以 Neo4j 以及其声明式查询语⾔ Cypher 为例，介绍图数据库查询处理的具体流程。
### 查询处理步骤
