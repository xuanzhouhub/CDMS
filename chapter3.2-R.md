...
生成逻辑执行计划之后，关系数据库系统还需要确定逻辑计划中每个算子在物理执行时采用的算法。每个关系算子通常有多种执行算法，如连接算子的算法有嵌套循环连接、排序合并连接、散列连接等，扫描算子的算法有全表扫描和索引扫描。一个逻辑执行计划有多种候选物理执行计划。关系数据库系统基于代价模型估计执行的代价，从众多候选物理执行计划中选择代价最小的计划。这里的代价最小是指磁盘访问次数最少，CPU执行开销最小。通过物理优化后的查询树称为物理计划树，也称为物理执行计划。在图3.7中，物理优化过程确定读取SC表使用全表扫描，读取Student表使用索引扫描，连接运算使用嵌套循环连接算法。
