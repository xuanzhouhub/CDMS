...
前一小节我们介绍了关系数据库的查询处理步骤以及基于火山模型的流水线式查询执行流程。本小节，我们主要介绍执行流程中各关系算子的实现算法并评估每种算法的I/O代价。
...
WHERE Sname = '沐辰' ;
```
上述查询语句中，Student表中的数据按分页的方式进行存储。表中的学生信息存放在多个页中，每个页中包含多个学生的信息。多个页通过Inode结构或者在学生表的主键属性（学号Sno）上的B+树聚簇索引结构进行组织。假设主键索引的层数为L，Student表占用了B个物理页。此外，假设在学生表的学生姓名属性（Sname）上构建了一个基于B+树的辅助索引，该索引的层树为L‘。

选择算子只涉及一张表，通常包含以下两种实现方法：

* 全表扫描（Table Scan）：首先基于Inode结构读取Student表的第一个物理页到内存，然后检查内存中的每个元组，如果该元组满足选择条件则输出，最后重复上述操作直到处理完Student的B个物理页。全表扫描的I/O代价为B，即cost=B，整个运算过程中需要读取B个物理页。
* 索引扫描（Index Scan）：首先基于Sname上的辅助索引找到“沐辰”所在的叶子结点，然后通过叶子结点上的指针找到数据对应的物理页。整个执行过程中，需要读取B+树中从根结点到叶子结点L’个物理块，以及读取查询结果所在的物理页数。假设有S个元组符合查询条件，也就是索引的选择基数为S，最坏的情况是S个元组存放在不同的物理页中，那么索引扫描的I/O代价为L+S，整个运算过程只需要读取L+S个物理页。如果选择条件是基于主键的等值查询，选择运算默认使用索引扫描，其I/O代价为L+1。
...
Repeat;
```
假设S表占据B(S)个物理页，R表占据B(R)个物理页，那么嵌套循环连接算法的I/O代价为：cost=B(S)+B(S)B(R)/(M-1)。该算法的执行代价比较高，尤其是对两张大表的等值连接。通常，采用嵌套循环连接算法时会选择较小的表（占用页数较少的表）作为左表S，这样可以减少算法的I/O代价。如果B(S)小于M-1，那么I/O代价仅为B(S)+B(R)。
...
索引连接算法是利用连接属性上的索引来实现连接操作。例3.4中，Student表（S表）在Sno上构建了B+树索引，SC表（R表）在Sno和Cno上构建了B+树索引结构。SC表上的索引称为组合索引，该索引的层数为L，索引在Sno上的选择基数为V（平均有V个元组具有相同的Sno值）。
