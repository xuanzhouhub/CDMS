...
前一小节我们介绍了查询处理步骤以及基于火山模型的流水线式查询执行流程。本小节，我们主要介绍各执行流程中关系算子的实现算法。
...
* 全表扫描（Table Scan）：首先基于Inode结构读取Student表的第一个物理页到内存，然后检查内存中的每个元组，如果该元组满足选择条件则输出，最后重复上述操作直到处理完Student的B个物理页。全表扫描的主要执行代价为B，即cost=B，整个运算过程中需要读取B个物理页。
