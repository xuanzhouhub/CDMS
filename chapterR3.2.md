...
前一小节我们介绍了查询计划的生成以及基于火山模型生成的流水线式处理流程。本小节，我们主要介绍各执行流程中查询算子的实现，如选择、投影、连接，并给出各算子的执行代价估算。
...
WHERE Sname = '沐辰' ;
```
上述查询语句中，Student表中的数据按分页的方式进行存储。表中的学生信息存放在多个页中，每个页中包含多个学生的信息。多个页通过Inode结构或者在学生表的主键属性（学号Sno）上的B+树聚簇索引结构进行组织。假设主键索引的层数为L，Student表占用了B个物理页。此外，我们在学生表的学生姓名属性（Sname）上构建了一个基于B+树的辅助索引，假设该索引的层树为L‘。

选择算子只涉及一张表，通常包含以下两种实现方法：

* 全表扫描（Table Scan）：首先基于Inode结构 读取Student表的第一个物理页到内存，然后检查内存中的每个元组，如果该元组满足选择条件则输出，最后重复上述操作直到处理完Student的B个物理页。全表扫描的主要执行代价为B，即cost=B，整个运算过程中需要读取B个物理页。
* 索引扫描（Index Scan）：首先基于Sname上的辅助索引找到“沐辰”所在的叶子结点，然后通过叶子结点上的指针找到数据对应的物理页。整个执行过程中，需要读取B+树中从根结点到叶子结点L’个物理块，以及读取查询结果所在的物理页数。假设有S个元组符合查询条件，也就是索引的选择基数为S，最坏的情况是S个元组存放在不同的物理页中，那么索引扫描的执行代价为L+S，整个运算过程只需要读取L+S个物理页。如果选择条件是基于主键的等值查询，选择运算默认使用索引扫描，其执行代价为L+1。
