...
例3.7的聚合查询中包含\$lookup（关联）、\$unwind（展开文档数组）、\$match（过滤）和\$project（投影）四个数据处理阶段，首先\$lookup（关联）阶段将student学生文档集和sc选课文档集进行左连接，以学号（sno）作为等值连接字段，将关联结果即每个学生的信息和学生选课信息存入selectedCourses字段，selectedCourses是文档数组，然后\$unwind（展开文档数组）阶段将selectedCourses数组进行展开，使得每一条学生选课信息成为一个独立的文档，之后\$match（过滤）阶段从展开后的文档中筛选出符合满足条件的文档，最后project（投影）阶段从过滤后的结果中提取\$selectedCourses.cno和\$selectedCourses.grade字段的值，构成新的只包含课程号和课程成绩的结果文档。例3.7在文档数据库系统中的查询处理过程如图3.12所示。
