...
逻辑等价变换规则主要包括连接、笛卡尔积交换律和结合律，选择和投影的交换律等。常用的关系代数表达式的逻辑等价交换规则如图3.8所示。
<center>
...
图3.9展示了上例SQL经查询解析生成的初始查询树，再经过代数优化的过程。整个代数优化过程基于启发式规则，主要包括分解复合的选择谓词、下推选择运算、将笛卡尔积转换为连接运算以及引入并下推投影运算。
...
物理优化在选择关系运算算法时需要遵循查询处理性能的衡量标准，即考虑个关系运算算法的I/O代价。优化方法包括基于启发式规则的优化、基于代价估算的优化和两者结合的优化方法。
...
[**上一页<<**](chapter3.3.md) | [**>>下一页**](chapter3.5-D.md)
